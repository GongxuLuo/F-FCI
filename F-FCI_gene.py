...
# def find_paths_with_colliders(matrix, start, end):
#     """
#     Find all undirected simple paths (allowing edge reversal) in a directed weighted graph,
#     from the start node to the end node, with at least 2 edges (i.e. ≥ 3 nodes in the path).

#     For each path, record:
#       - 'node_seq': the sequence of nodes in the path.
#       - 'edge_weights': the corresponding weights along the path.
#           * When an edge is traversed in the forward direction (i.e. matrix[u][v] != 0),
#             use that weight.
#           * When an edge is traversed in reverse (i.e. matrix[v][u] != 0),
#             use that weight.
    
#     A node z (an intermediate node, not the start or end) is considered a collider
#     on the path if for its immediate neighbors x and y in the path (i.e. ... x, z, y, ...):
#       - The edge from x to z was traversed in the forward direction (x → z), and
#       - The edge from y to z was traversed in reverse (i.e. the actual edge is y → z,
#         but on the path we traveled from z to y because we reversed that edge).
    
#     The function returns:
#       - detailed_paths: a list of dicts, each with keys:
#             'node_seq'    : list of nodes along the path,
#             'edge_weights': list of corresponding edge weights.
#       - all_intermediate_nodes: union (list) of all intermediate nodes (excluding start and end) across all paths.
#       - all_colliders: union (list) of all collider nodes found across all paths.
#     """
#     num_nodes = len(matrix)
#     # Each found path is recorded as a tuple:
#     # (node_seq, directions, edge_weights)
#     # where directions is a list of the same length as edge_weights.
#     # Each entry in directions is 'F' (forward) or 'R' (reverse).
#     all_paths = []
    
#     def dfs(current, path, directions, edge_weights):
#         if current == end:
#             if len(path) >= 3:
#                 # Only record paths with at least 2 edges.
#                 all_paths.append((list(path), list(directions), list(edge_weights)))
#             return
#         for nxt in range(num_nodes):
#             if nxt in path:
#                 continue
#             # Check for a forward edge from current to nxt.
#             if matrix[current][nxt] != 0:
#                 path.append(nxt)
#                 directions.append('F')
#                 edge_weights.append(matrix[current][nxt])
#                 dfs(nxt, path, directions, edge_weights)
#                 edge_weights.pop()
#                 directions.pop()
#                 path.pop()
#             # Otherwise, check for a reverse edge from nxt to current.
#             elif matrix[nxt][current] != 0:
#                 path.append(nxt)
#                 directions.append('R')
#                 edge_weights.append(matrix[nxt][current])
#                 dfs(nxt, path, directions, edge_weights)
#                 edge_weights.pop()
#                 directions.pop()
#                 path.pop()
...
#     # For each path, check each intermediate node (i.e. path[i] for i=1..len(path)-2)
#     # Let left_dir = directions[i-1] (edge from path[i-1] to path[i])
#     # and right_dir = directions[i] (edge from path[i] to path[i+1]).
#     # If left_dir is 'F' (meaning path[i-1] → path[i]) and
#     #    right_dir is 'R' (meaning we traversed the edge in reverse, so the actual edge is path[i+1] → path[i]),
#     # then path[i] is a collider.
#     for node_seq, dirs, weights in all_paths:
#         colliders = []
#         # The intermediate nodes are those at indices 1 to len(node_seq)-2.
#         for i in range(1, len(node_seq)-1):
#             all_intermediate_nodes.add(node_seq[i])
#             if dirs[i-1] == 'F' and dirs[i] == 'R':
#                 colliders.append(node_seq[i])
#                 all_colliders.add(node_seq[i])
#         detailed_paths.append({
#             'node_seq': node_seq,
#             'edge_weights': weights
#         })
    
#     return detailed_paths, list(all_intermediate_nodes), list(all_colliders)
...
data = pd.read_csv('./magic/norman_MAGIC_7.csv')
ske = data.values
rows, cols = ske.shape
for i in range(rows):
    for j in range(cols):
...
# import pdb
# pdb.set_trace()
dag = refine_L_C(dag, data_final, L_C)
dag = dag_transformation(dag)
dag_n = correct_inducing_path(dag, data_final)
dag = dag_transformation(dag_n)


print(result['latent'])
print(result['selection'])
per_dag = dag[np.ix_(per_index, per_index)]
# np.save('./CDIP_new/output_IAG.npy', dag_n[np.ix_(per_index, per_index)])
np.save('./CDIP_new/latent_m.npy', np.array(result['latent']))
np.save('./CDIP_new/selection_m.npy', np.array(result['selection']))
np.save('./CDIP_new/gene_name_kci_m.npy', gene_name)
np.save('./CDIP_new/per_index_kci_m.npy', per_index)
np.save('./CDIP_new/output_dag_kci_m.npy', dag)
np.save('./CDIP_new/per_dag_kci_m.npy', per_dag)
np.save('./CDIP_new/per_name_kci_m.npy',np.array(perturbation))